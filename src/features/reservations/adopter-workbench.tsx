...
export function AdopterWorkbench({ records, role, initial }: { records: AdopterWorkbenchRecord[]; role: WorkbenchRole; initial: InitialFilters }) {
  ...
  const panel = selected ? <AdopterPanel journey={selected} role={role} returnPath={returnPath} hasPrevious={selectedIndex > 0} hasNext={selectedIndex < visible.length - 1} onClose={() => setSelectedId(null)} onPrevious={() => setSelectedId(visible[selectedIndex - 1]?.record.id ?? null)} onNext={() => setSelectedId(visible[selectedIndex + 1]?.record.id ?? null)} /> : <div className="py-20 text-center"><p className="font-semibold">Sélectionnez une famille</p><p className="mt-2 text-sm text-muted">Ses jalons, actions et événements apparaîtront ici.</p></div>;
...
function AdopterPanel({ journey, role, returnPath, hasPrevious, hasNext, onClose, onPrevious, onNext }: { journey: AdopterJourney; role: WorkbenchRole; returnPath: string; hasPrevious: boolean; hasNext: boolean; onClose: () => void; onPrevious: () => void; onNext: () => void }) {
  ...
  return <div>
    ...
    <div className="mt-5 space-y-2">
      <Detail title="Famille et profil"><dl className="grid gap-3 text-sm sm:grid-cols-2"><Info label="Email" value={record.email ?? "Non renseigné"} /><Info label="Téléphone" value={record.phone ?? "Non renseigné"} /><Info label="Référence" value={record.reference} /><Info label="Rôle connecté" value={role} /></dl>{record.contactId ? <Link href={`/contacts/${record.contactId}`} className="mt-3 inline-flex text-sm font-semibold text-accent hover:underline">Ouvrir la fiche contact</Link> : null}</Detail>
      <Detail title="Position et portée"><p className="text-sm">{journey.scopeLabel} · {queueLabels[journey.queue]} · {record.rank ? `rang #${record.rank}` : "rang à compléter"}</p><p className="mt-2 text-xs text-muted">Le positionnement global reste indisponible jusqu’au lot spécialisé.</p></Detail>
