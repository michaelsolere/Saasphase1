...
function AdopterPanel({ journey, role, returnPath, hasPrevious, hasNext, onOpenProfile, onClose, onPrevious, onNext }: { journey: AdopterJourney; role: WorkbenchRole; returnPath: string; hasPrevious: boolean; hasNext: boolean; onOpenProfile: () => void; onClose: () => void; onPrevious: () => void; onNext: () => void }) {
  ...
  return <div>
    ...
    <div className="mt-5 space-y-2">
      ...
      <Detail title="Paiements"><p className="text-sm">Reçu : {(record.paidCents / 100).toLocaleString("fr-FR")} € · Remboursé : {(record.refundedCents / 100).toLocaleString("fr-FR")} €</p><QuickMutationLink canMutate={canMutate} href={`/reservations/${record.id}?tab=finances&return_to=${encodeURIComponent(returnPath)}#payments`} label="Enregistrer un paiement" /></Detail>
      <Detail title="Documents"><p className="text-sm">{record.signedDocumentCount}/{record.documentCount} document(s) signé(s) ou validé(s).</p>{canMutate ? <Link href={`/reservations/${record.id}/preparer?return_to=${encodeURIComponent(returnPath)}`} className="mt-3 inline-flex rounded-lg bg-accent px-3 py-2 text-sm font-semibold !text-white">Préparer la réservation</Link> : <span className="mt-3 inline-flex cursor-not-allowed rounded-lg border px-3 py-2 text-sm font-semibold text-muted" title="Rôle owner ou admin requis">Préparer la réservation · owner/admin requis</span>}<Link href={`/reservations/${record.id}?tab=finances&return_to=${encodeURIComponent(returnPath)}#documents`} className="ml-3 mt-3 inline-flex text-sm font-semibold text-accent">Ouvrir les documents</Link></Detail>
      <Detail title="Rendez-vous et attribution"><p className="text-sm">Choix : {formatDate(record.choiceAppointmentAt)}</p><p className="mt-1 text-sm">Départ : {formatDate(record.departureAppointmentAt)}</p><QuickMutationLink canMutate={canMutate} href={`/reservations/${record.id}?tab=etapes&return_to=${encodeURIComponent(returnPath)}#appointments`} label="Créer ou modifier un rendez-vous" /></Detail>
