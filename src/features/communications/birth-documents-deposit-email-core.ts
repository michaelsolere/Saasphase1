...
import { normalizeTransactionalEmailAttachmentSnapshotJson } from "@/features/communications/transactional-email-attachments";
...
export function deriveBirthDocumentsDepositPaymentPlan(input: {
  paidCents: number;
  preReservationDepositCents: number;
  completeDepositCents: number;
}) {
  if (input.paidCents < input.preReservationDepositCents) {
    return {
      eligible: false as const,
      errorCode: "pre_reservation_unpaid" as const,
    };
  }
  const complementCents = Math.max(
    0,
    input.completeDepositCents - input.paidCents,
  );
  return {
    eligible: true as const,
    complementCents,
    paymentRequired: complementCents > 0,
  };
}
...
export async function sendBirthDocumentsDepositEmailForReservation(
  ...
  const { data: deliveryReservation, error: deliveryReservationError } =
    await options.supabase
      ...
      .is("deleted_at", null)
      .maybeSingle();
  if (deliveryReservationError || !deliveryReservation) {
  ...
  const result = await runTransactionalCampaignDelivery({
    ...
    context: {
      organizationId: deliveryReservation.organization_id,
      roles: ["owner", "admin"],
    },
    ...
    prepareOperation: async ({ supabase, organizationId }) => {
      ...
      const paymentPlan = deriveBirthDocumentsDepositPaymentPlan({
        paidCents: paid,
        preReservationDepositCents: settings.preReservationDepositCents,
        completeDepositCents: settings.completeDepositCents,
      });
      if (!paymentPlan.eligible) return { ok: false, errorCode: paymentPlan.errorCode };
    ...
    prepareClaimedOperation: async ({ supabase, organizationId, userId, operation, attempt }) => {
      ...
      if (!paymentPlan.eligible) return { ok: false, errorCode: paymentPlan.errorCode };
      const amount = paymentPlan.complementCents;
      ...
      let payment = paymentPlan.paymentRequired
        ? active.find(p => compatible(p, amount))
        : undefined;
      if (paymentPlan.paymentRequired && active.length && !payment) {
        return { ok: false, errorCode: "incompatible_request" };
      }
      let action: "created" | "reactivated" | "reused" | undefined = payment
        ? "reused"
        : undefined;
      ...
      if (paymentPlan.paymentRequired && !payment) {
      ...
      const variables = {
        ...buildBirthDocumentsDepositVariables({ firstName: data.contact.first_name, lastName: data.contact.last_name, fullName, litterName: data.litter.name, litterGroupName: data.overview?.litter_group_name ?? null, motherName: data.overview?.mother_display_name ?? null, fatherName: data.overview?.father_display_name ?? null, birthDate: data.litter.actual_birth_date, desiredSexPreference: data.desiredSex, paidArrhesCents: paid, complementAmountCents: payment?.amount_cents ?? 0, complementDueDate: payment?.due_date ?? null, completeDepositCents: settings.completeDepositCents, organizationName: data.organizationName }),
        ...(payment ? { payment_request_id: payment.id } : {}),
      };
      const compensate = !payment || action === "reused" ? undefined : async () => {
  ...
  if (result.outcome === "already_sent" && result.attemptId) {
    const { data: attempt, error: attemptError } = await options.supabase
      .from("email_delivery_attempts")
      .select("attachments_snapshot")
      .eq("organization_id", deliveryReservation.organization_id)
      .eq("reservation_id", input.reservationId)
      .eq("id", result.attemptId)
      .eq("status", "sent")
      .is("deleted_at", null)
      .maybeSingle();
    const manifest = normalizeTransactionalEmailAttachmentSnapshotJson(
      attempt?.attachments_snapshot,
    );
    const commitment = manifest?.find(
      (entry) => entry.document_type === "commitment_certificate",
    );
    const contract = manifest?.find(
      (entry) => entry.document_type === "reservation_contract",
    );
    if (!attemptError && (!manifest || manifest.length === 0)) {
      return mapResult(result);
    }
    if (attemptError || !commitment || !contract || manifest?.length !== 2) {
      return {
        status: "failed",
        deliveryState: "uncertain",
        attemptId: result.attemptId,
        errorCode: "delivery_reconciliation_failed",
      };
    }
    const { data, error } = await options.supabase.rpc(
      "mark_birth_documents_deposit_documents_sent",
      {
        p_organization_id: deliveryReservation.organization_id,
        p_reservation_id: input.reservationId,
        p_commitment_document_id: commitment.document_id,
        p_contract_document_id: contract.document_id,
        p_commitment_file_sha256: commitment.file_sha256,
