...
export function AnimalProfileView(props: AnimalProfileViewProps) {
  ...
  function openIdentityDialog() {
    const trigger = document.getElementById("animal-final-identity-trigger");
    if (trigger) {
      trigger.click();
      return;
    }
    const identitySection = document.getElementById("animal-essential-identity");
    identitySection?.scrollIntoView({ block: "start", behavior: "smooth" });
    identitySection?.focus();
  }
  ...
  return <main className="min-h-screen w-full overflow-x-hidden" data-testid="animal-profile">
    <div className="mx-auto w-full max-w-[80rem] px-4 py-7 sm:px-8 lg:px-10 lg:py-9">
      ...
      <section role="tabpanel" id={`animal-panel-${props.initialTab}`} aria-labelledby={`animal-tab-${props.initialTab}`} tabIndex={0} className="py-7 outline-none focus-visible:ring-2 focus-visible:ring-accent">
        ...
        {props.initialTab === "health" ? <AnimalHealthSection notes={props.healthNotes} events={props.healthEvents} documents={props.healthDocuments} onAdd={canWrite ? <HealthEventDialog animalId={animal.id} /> : null} onDocuments={() => openTab("documents")} hasError={props.errors.health} /> : null}
