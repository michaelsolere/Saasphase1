...
import { LitterCareTimelinePanel } from "./litter-care-timeline-panel";
import {
  getLitterCareTimelineZoom,
...
/** Grid views shared with the organization breeding calendar. */
export type LitterCareCalendarGridView = "month" | "week" | "agenda";

/** Litter journal calendar page views, including the biological timeline. */
export type LitterCareCalendarPageView = LitterCareCalendarGridView | "timeline";

/** @deprecated Prefer LitterCareCalendarGridView for global calendar usage. */
export type LitterCareCalendarView = LitterCareCalendarGridView;
...
const pageViews: LitterCareCalendarPageView[] = ["timeline", "month", "week", "agenda"];
...
function adjacentWeek(date: string, difference: number) {
  ...
  return civilDate(parts.year, parts.month, parts.day + difference * 7);
}
...
function query({
  ...
  litter: string;
  view?: LitterCareCalendarPageView;
  month?: string;
  date?: string;
  kind: string;
  category: string;
  ...
  if (view === "timeline" && zoom && zoom !== "cycle") params.set("zoom", zoom);
  return `/litters/journal/calendar?${params.toString()}`;
}

function viewLabel(view: LitterCareCalendarPageView) {
  if (view === "timeline") return "Frise";
  if (view === "week") return "Semaine";
  if (view === "agenda") return "Agenda";
  return "Mois";
}

export function LitterCareCalendarCard({
  item,
  litterId,
  contextLabel,
  agenda = false,
}: {
  item: LitterCareCalendarItem;
  litterId?: string;
  contextLabel?: string;
  agenda?: boolean;
}) {
  ...
  const showTitle =
    ...
    item.windowPosition === "end";
  const fullWindow =
    agenda && item.kind === "window" && item.retainedStartsOn && item.retainedEndsOn
      ? `Du ${item.retainedStartsOn}${item.task.retainedStartsLocalTime ? ` à ${item.task.retainedStartsLocalTime.slice(0, 5)}` : ""} au ${item.retainedEndsOn}${item.task.retainedEndsLocalTime ? ` à ${item.task.retainedEndsLocalTime.slice(0, 5)}` : ""}`
      : null;
  return (
    <Link
      href={`/litters/journal?litter=${encodeURIComponent(litterId ?? item.task.litterId)}#litter-care-tasks`}
      className={`block rounded border border-accent/40 bg-surface px-2 py-1.5 text-xs text-foreground hover:bg-accent/10 ${border}`}
    >
      ...
      {item.task.isScheduleLocked ? (
        <span className="block font-medium" aria-label="Programmation verrouillée">
          🔒 Verrouillée
        </span>
      ) : null}
...
export function LitterCareCalendarMonthView({
  calendar,
  litterId,
  contextLabels,
}: {
  calendar: LitterCareCalendar;
  litterId?: string;
  contextLabels?: Record<string, string>;
}) {
  return (
    <section
      aria-label={`Calendrier ${monthLabel(calendar.month)}`}
      className="max-w-full overflow-x-auto rounded-2xl border bg-surface"
    >
      <div className="grid min-w-[980px] grid-cols-7">
        {weekdays.map((day) => (
          <div key={day} className="border-b px-3 py-3 text-center text-sm font-semibold">
            {day}
          </div>
        ))}
        {calendar.days.map((day) => (
          ...
          >
            ...
            <div className="space-y-1.5">
              {day.items.map((item) => (
                <LitterCareCalendarCard
                  key={`${item.task.id}:${day.date}`}
                  item={item}
                  litterId={litterId}
                  contextLabel={contextLabels?.[item.task.litterId]}
                />
              ))}
            </div>
          </article>
...
export function LitterCareCalendarWeekView({
  calendar,
  litterId,
  contextLabels,
}: {
  ...
  return (
    <section
      aria-label={`Semaine du ${calendar.startsOn} au ${calendar.endsOn}`}
      className="max-w-full overflow-x-auto rounded-2xl border bg-surface"
    >
      <div className="grid min-w-[980px] grid-cols-7">
...
export function LitterCareCalendarAgendaView({
  calendar,
  litterId,
  contextLabels,
}: {
  ...
  const days = calendar.days
    .map((day) => ({
      ...day,
      items: sortLitterCareAgendaItems(
...
export function LitterCareCalendarPanel({
  litters,
  litter,
  details = null,
  tasks,
  ...
  const monthCalendar = projectLitterCareCalendar({
    tasks,
  ...
  const weekCalendar = projectLitterCareCalendarWeek({
    tasks,
    requestedDate,
    todayDate,
    todayLocalTime,
  ...
  const dateForView =
    view === "timeline"
      ? date ?? todayDate
      : view === "month"
        ? `${monthCalendar.month}-01`
        : weekCalendar.startsOn;
  const navigation = { litter: litterId, kind, category };
  const exportHref = `/litters/journal/calendar/export?${new URLSearchParams({ litter: litterId, kind, category }).toString()}`;

  const preservedSearchParams = {
    view: view === "month" ? undefined : view,
    month: view === "timeline" ? undefined : monthCalendar.month,
    date: dateForView,
    zoom: view === "timeline" && zoom !== "cycle" ? zoom : undefined,
    kind,
    category,
  };

  const litterSelector = (
    <LitterJournalSelector
      litters={litters}
      selectedLitterId={litterId}
      basePath="/litters/journal/calendar"
          ...
          href={query({
            ...
            month:
              candidate === "month"
                ? view === "month"
                  ? monthCalendar.month
                  : (dateForView.slice(0, 7) || monthCalendar.month)
                : monthCalendar.month,
            date: candidate === "month" ? undefined : dateForView,
            zoom: candidate === "timeline" ? zoom : undefined,
          })}
        >
          {viewLabel(candidate)}
        </Link>
  ...
  const filters = (
    <form
      method="get"
      className="flex flex-col gap-4 rounded-2xl border bg-surface p-5 sm:flex-row sm:items-end sm:p-6"
    >
      <input type="hidden" name="litter" value={litterId} />
      <input type="hidden" name="view" value={view} />
      {view === "timeline" ? (
        <>
          {zoom !== "cycle" ? <input type="hidden" name="zoom" value={zoom} /> : null}
          <input type="hidden" name="date" value={dateForView} />
        </>
      ) : (
        <>
      ...
      <Link
        href={`/litters/journal/calendar?litter=${encodeURIComponent(litterId)}${view === "timeline" ? "&view=timeline" : ""}`}
        className="rounded-lg border px-4 py-2 text-center text-sm font-semibold hover:bg-muted"
      >
        Réinitialiser
      </Link>
    </form>
  ...
  const exportPanel = (
    <div className="rounded-2xl border bg-surface p-5 text-sm">
      <Link href={exportHref} className="inline-flex rounded-lg border px-4 py-2 font-semibold hover:bg-muted">
        Télécharger le fichier iCalendar
      </Link>
      <p className="mt-2 text-muted">
        Fichier à importer manuellement dans un agenda externe ; l’abonnement synchronisé n’est
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6">
      <header className="rounded-2xl border bg-surface p-5 sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-lg capitalize text-muted">
              {view === "month"
                ? monthLabel(monthCalendar.month)
                : `Semaine du ${weekCalendar.startsOn} au ${weekCalendar.endsOn}`}
            </p>
            <Link
        ...
        {viewControls}
        {view === "month" ? (
          <nav aria-label="Navigation mensuelle" className="mt-3 flex flex-wrap gap-3">
            <Link
              className="rounded-lg border px-3 py-2 text-sm font-medium hover:bg-muted"
              href={query({ ...navigation, month: adjacentMonth(monthCalendar.month, -1) })}
            >
              Mois précédent
            </Link>
            <Link
              className="rounded-lg border px-3 py-2 text-sm font-medium hover:bg-muted"
              href={query({ ...navigation, month: todayDate.slice(0, 7) })}
            >
              Aujourd’hui
            </Link>
            <Link
          ...
          <nav aria-label="Navigation hebdomadaire" className="mt-3 flex flex-wrap gap-3">
            <Link
              className="rounded-lg border px-3 py-2 text-sm font-medium hover:bg-muted"
              href={query({
                ...navigation,
                view,
                month: weekCalendar.startsOn.slice(0, 7),
      ...
      {!calendar.hasPlannedItems ? (
        ...
        >
          Aucun élément planifié pour cette portée.
        </p>
      ) : null}
      ...
      {view === "month" ? (
        <LitterCareCalendarMonthView calendar={monthCalendar} litterId={litterId} />
      ) : view === "week" ? (
        <LitterCareCalendarWeekView calendar={weekCalendar} litterId={litterId} />
      ) : (
        <LitterCareCalendarAgendaView calendar={weekCalendar} litterId={litterId} />
      )}
    </main>
  );
}
