...
import { createLitterPlanningModelAction } from "@/features/settings/litter-planning-models-actions";
import { loadLitterPlanningModelEditorPage } from "@/features/settings/litter-planning-models-presentation";
import { createClient } from "@/lib/supabase/server";

export const dynamic = "force-dynamic";

export default async function NewLitterPlanningModelPage() {
  ...
  if (membership.error || !membership.data?.organization_id) {
    redirect("/settings/litter-planning-models");
  }
  ...
  if (page.outcome === "error") {
    ...
    if (page.code === "forbidden") redirect("/settings/litter-planning-models");
    return (
  ...
  const saveAction = createLitterPlanningModelAction.bind(null, {
