-- Atomically correct the business metadata of an active, non-recurring ad-hoc item.
create or replace function public.update_litter_plan_ad_hoc_item_metadata(
  p_litter_id uuid,
  p_litter_plan_item_id uuid,
  p_client_command_id uuid,
  p_expected_plan_revision integer,
  p_expected_item_revision integer,
...
begin
  ...
  if v_user is null or p_litter_id is null or p_litter_plan_item_id is null or p_client_command_id is null
    ...
    or p_metadata->>'version' <> '1' or p_metadata->>'operation' <> 'update_metadata'
  then return next; return; end if;

  v_title := btrim(p_metadata->>'title');
  v_description := case when p_metadata->>'description' is null then null else btrim(p_metadata->>'description') end;
  ...
  then return next; return; end if;
  v_payload := jsonb_build_object('version',1,'operation','update_metadata','title',v_title,'description',v_description,'category',v_category,'targetScope',v_target_scope,'priority',v_priority);

  select l.organization_id into v_org from public.litters l
  where l.id=p_litter_id and l.deleted_at is null;
  if v_org is null then reason := 'not_found'; return next; return; end if;
  if not public.has_organization_role(v_org, array['owner','admin','member']) then
    reason := 'membership_required'; return next; return;
  end if;

  select * into v_command from public.litter_plan_ad_hoc_commands c
  where c.organization_id=v_org and c.client_command_id=p_client_command_id for update;
  if found then
    if v_command.payload is distinct from v_payload then reason := 'client_command_conflict'; return next; return; end if;
    outcome:=v_command.outcome; reason:=v_command.reason; litter_plan_id:=v_command.litter_plan_id; plan_revision:=v_command.result_plan_revision;
    litter_plan_item_id:=v_command.litter_plan_item_id; task_id:=v_command.task_id; result:=v_command.result; replayed:=true;
    item_revision := case when result ? 'itemRevision' then (result->>'itemRevision')::integer else null end;
    task_revision := case when result ? 'taskRevision' then (result->>'taskRevision')::integer else null end;
    return next; return;
  end if;
  ...
  select * into v_item from public.litter_plan_items i where i.organization_id=v_org and i.litter_id=p_litter_id and i.id=p_litter_plan_item_id for update;
  if v_plan.id is null or not found or v_plan.id is distinct from v_item.litter_plan_id or v_item.origin_kind <> 'ad_hoc'
    or v_item.item_kind not in ('milestone','task','window') or v_item.materialization_state <> 'materialized'
    or v_item.source_planning_model_id is not null or v_item.organization_template_id is not null
  then reason:='not_found'; return next; return; end if;
  if v_plan.revision<>p_expected_plan_revision or v_item.revision_no<>p_expected_item_revision then
    reason:='stale_revision'; litter_plan_id:=v_plan.id; plan_revision:=v_plan.revision; litter_plan_item_id:=v_item.id; item_revision:=v_item.revision_no; return next; return; end if;
  select * into v_task from public.litter_care_tasks t where t.organization_id=v_org and t.litter_id=p_litter_id and t.litter_plan_item_id=v_item.id for update;
  if not found or v_task.source<>'manual' or v_task.status<>'planned' or v_task.litter_plan_series_id is not null then reason:='not_found'; return next; return; end if;
  if (select count(*) from public.litter_care_tasks t where t.organization_id=v_org and t.litter_plan_item_id=v_item.id) <> 1 then reason:='not_found'; return next; return; end if;
  ...
  update public.litter_plan_items i set title=v_title,description=v_description,category=v_category,target_scope=v_target_scope,priority=v_priority,revision_no=i.revision_no+1,updated_by=v_user where i.id=v_item.id returning * into v_item;
  update public.litter_care_tasks t set title=v_title,description=v_description,category=v_category,target_scope=v_target_scope,priority=v_priority,revision_no=t.revision_no+1,updated_by=v_user where t.id=v_task.id returning * into v_task;
  update public.litter_plans p set revision=p.revision+1,updated_by=v_user where p.id=v_plan.id returning * into v_plan;
  result:=jsonb_build_object('kind',v_item.item_kind,'itemRevision',v_item.revision_no,'taskRevision',v_task.revision_no);
  insert into public.litter_plan_ad_hoc_commands(organization_id,litter_id,litter_plan_id,litter_plan_item_id,task_id,client_command_id,payload,outcome,result,result_plan_revision,created_by)
  values(v_org,p_litter_id,v_plan.id,v_item.id,v_task.id,p_client_command_id,v_payload,'success',result,v_plan.revision,v_user);
  outcome:='success'; reason:=null; litter_plan_id:=v_plan.id; plan_revision:=v_plan.revision; litter_plan_item_id:=v_item.id; item_revision:=v_item.revision_no; task_id:=v_task.id; task_revision:=v_task.revision_no; return next;
exception when others then
  raise exception 'metadata update failed' using errcode='P0001';
end;
$fn$;
...
-- The function above is replaced below in the same non-applied migration.  This
-- version keeps the complete intent in the command log and validates JSON types
-- before any text conversion.
create or replace function public.update_litter_plan_ad_hoc_item_metadata(
...
begin
  ...
  select count(*) into v_task_count from public.litter_care_tasks where organization_id=v_org and litter_plan_item_id=v_item.id;
  select * into v_task from public.litter_care_tasks where organization_id=v_org and litter_plan_item_id=v_item.id for update;
