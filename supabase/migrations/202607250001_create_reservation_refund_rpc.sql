-- Transactional reservation refund with overpayment guard.
-- Locks the reservation row, recomputes paid/refunded aggregates
-- (same rules as reservation_overview), and inserts payment_type=refund
-- only when the requested amount does not exceed the remaining refundable
-- balance. Concurrent callers serialize on the reservation lock.
