-- RESERVATION-PREPARATION-01
-- Document delivery is a sensitive operational mutation. Keep the existing
-- atomic validation while aligning SQL authorization with the adopter workbench.

drop function if exists public.mark_birth_documents_deposit_documents_sent(
...
begin
  ...
  if not exists (
    ...
    where attempt.organization_id = p_organization_id
      ...
      and jsonb_array_length(attempt.attachments_snapshot) = 2
      and attempt.attachments_snapshot @> jsonb_build_array(jsonb_build_object(
        'kind', 'document_pdf',
        'document_id', p_commitment_document_id::text,
        'document_type', 'commitment_certificate',
        'file_sha256', p_commitment_file_sha256,
        'file_size_bytes', p_commitment_file_size_bytes,
        'version', p_commitment_version
      ))
      and attempt.attachments_snapshot @> jsonb_build_array(jsonb_build_object(
        'kind', 'document_pdf',
        'document_id', p_contract_document_id::text,
        'document_type', 'reservation_contract',
        'file_sha256', p_contract_file_sha256,
        'file_size_bytes', p_contract_file_size_bytes,
        'version', p_contract_version
      ))
  ) then
    raise exception 'sent delivery attempt required' using errcode = '23514';
  end if;
  ...
  loop
    if v_document.reservation_id is distinct from p_reservation_id
      ...
      or v_document.status not in ('to_generate', 'sent')
    ...
    if v_document.status = 'to_generate' then
  ...
  if v_pending_count = 0 and v_sent_count = 2 then
    return 'sent';
  end if;

  if v_pending_count <> 2 then
    raise exception 'partial document delivery state' using errcode = '40001';
  end if;
...
revoke all on function public.mark_birth_documents_deposit_documents_sent(
  uuid, uuid, uuid, uuid, text, text, bigint, bigint, integer, integer, uuid, timestamptz
) from public;
