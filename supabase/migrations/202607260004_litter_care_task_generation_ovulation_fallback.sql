-- Align simple task generation with biological ovulation fallback:
-- estimated_ovulation absent + mating_date present → mating_date - 1 day.
-- Does not mutate litters.estimated_ovulation_date. Never uses mating_date_2.
-- Composite apply_litter_planning_model already implements first_mating_minus_24h.
...
begin
  ...
  if not v_stale then
    ...
    loop
      ...
      -- estimated_ovulation: explicit date first, else first mating − 1 day.
      -- Never fall back to mating_date_2. Do not write the derived date to litters.
      v_anchor_date := case v_template.anchor_type
        when 'first_mating' then v_litter.mating_date
        when 'estimated_ovulation' then coalesce(
          v_litter.estimated_ovulation_date,
          case when v_litter.mating_date is not null then v_litter.mating_date - 1 else null end
        )
...
comment on function public.generate_litter_care_tasks_from_plan(uuid, uuid, jsonb) is
  'Atomically revalidates an exact server plan and creates only its missing organization-template tasks. estimated_ovulation falls back to first mating − 1 day when no explicit ovulation date is set.';
