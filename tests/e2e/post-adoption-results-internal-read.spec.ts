...
function cleanup() {
  sql(`
    ...
    delete from public.post_adoption_questionnaire_response_revisions
      where id = ${q(ids.revision)}::uuid;
    delete from public.post_adoption_questionnaire_instances
      where id in (${q(ids.instance)}::uuid, ${q(ids.otherInstance)}::uuid);
    delete from public.reservations
      where id in (${q(ids.reservation)}::uuid, ${q(ids.otherReservation)}::uuid);
    delete from public.animals
      where id in (${q(ids.animal)}::uuid, ${q(ids.otherAnimal)}::uuid);
...
function remainingCounts() {
  return JSON.parse(
    sql(`select json_build_object(
      'revisions', (select count(*) from public.post_adoption_questionnaire_response_revisions where id = ${q(ids.revision)}::uuid),
      'instances', (select count(*) from public.post_adoption_questionnaire_instances where id in (${q(ids.instance)}::uuid, ${q(ids.otherInstance)}::uuid)),
      'reservations', (select count(*) from public.reservations where id in (${q(ids.reservation)}::uuid, ${q(ids.otherReservation)}::uuid)),
      'animals', (select count(*) from public.animals where id in (${q(ids.animal)}::uuid, ${q(ids.otherAnimal)}::uuid)),
...
function createFixture() {
  sql(`
    ...
    select ${q(ids.revision)}::uuid, ${q(organizationId)}::uuid, ${q(ids.instance)}::uuid, 1,
      definition_sha256, '{"t1_final_comment":"Texte libre E2E"}'::jsonb,
  ...
  try {
    ...
    expect(individual.data).toEqual([
      expect.objectContaining({
        ...
        latest_answers: { t1_final_comment: "Texte libre E2E" },
      }),
    ]);
    ...
    await expect(page.getByText("T1 : 1 questionnaire reçu pour 1 chiot concerné")).toBeVisible();
    ...
    await expect(page.getByText("Nova Résultats E2E")).toBeVisible();
    await expect(page.getByText("Réponse reçue", { exact: true })).toBeVisible();
    await expect(page.getByText("Jalon absent", { exact: true })).toBeVisible();

    const individualLink = page.getByRole("link", { name: "Voir les résultats individuels" });
