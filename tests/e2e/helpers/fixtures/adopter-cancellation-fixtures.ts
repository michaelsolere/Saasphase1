...
export async function createTestAdopterCancellationReadyScenario(
  execute: SqlExecutor,
  registry: Registry,
  input: {
  ...
  if (!reservation || reservation.organizationId !== input.organizationId) {
    throw new Error("E2E cancellation journey belongs to another organization");
  }

  if (JSON.stringify(input) !== JSON.stringify(snapshot)) {
    throw new Error("E2E cancellation scenario mutated its input");
  }
...
/**
 * Cancellation only updates the existing reservation row (status → cancelled).
 * No payment, document, animal or contact_role rows are created by the action.
 * This helper re-reads the cancelled journey and registers any roles already
 * present on the contact so cleanup stays complete if discovery finds extras.
 */
export async function registerActualCancellationEffects(
  execute: SqlExecutor,
  registry: Registry,
  input: {
    organizationId: string;
...
/**
 * Expiration only updates the existing reservation row (status → expired).
 * Same discovery surface as cancellation — no new financial or document rows.
 */
export async function registerActualExpirationEffects(
  execute: SqlExecutor,
  registry: Registry,
  input: {
    organizationId: string;
...
/**
 * Withdrawal only updates the existing reservation row (status → withdrawn).
 * Distinct from cancellation and expiration by final status only.
 */
export async function registerActualWithdrawalEffects(
  execute: SqlExecutor,
  registry: Registry,
  input: {
    organizationId: string;
