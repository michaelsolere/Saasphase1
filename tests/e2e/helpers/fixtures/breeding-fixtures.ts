...
export async function createTestLitter(execute: SqlExecutor, registry: Registry, input: Base & { motherId:string } & Partial<{name:string;fatherId:string;status:string;actualBirthDate:string}>) { const entityId=id(input.id); return insert(execute,"litters",{id:entityId,organization_id:input.organizationId,name:input.name ?? `E2E litter ${registry.namespace}`,species:"dog",breed:"Golden Retriever",mother_id:input.motherId,father_id:input.fatherId ?? null,status:input.status ?? "birth_expected",actual_birth_date:input.actualBirthDate ?? null,created_by:input.ownerId,updated_by:input.ownerId},registry); }
