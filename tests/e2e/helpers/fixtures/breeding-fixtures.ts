...
export async function createTestMembership(execute: SqlExecutor, registry: Registry, input: { id?: string; organizationId: string; profileId: string; role?: "owner" | "admin" | "member" | "viewer"; createdAt?: string }) { const entityId=id(input.id); return insert(execute,"memberships",{id:entityId,organization_id:input.organizationId,profile_id:input.profileId,role:input.role ?? "owner",status:"active",created_at:input.createdAt ?? new Date().toISOString(),created_by:input.profileId,updated_by:input.profileId},registry); }
...
export const createResolvedLitterCareTask = (execute:SqlExecutor, registry:Registry, input:TaskInput & {resolvedAt?:string}) => task(execute,registry,input,{status:"done",resolution_command_id:randomUUID(),resolved_at:input.resolvedAt ?? new Date().toISOString(),resolved_timezone_name:"Europe/Paris",resolved_by:input.ownerId,resolution_note:input.resolutionNote ?? null});
