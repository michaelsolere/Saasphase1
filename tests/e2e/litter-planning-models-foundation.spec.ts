...
function cleanup() {
  sql(`
    delete from public.litter_planning_model_commands where model_id::text like '9f240001-%' or client_command_id::text like '9f240001-%';
    delete from public.litter_planning_model_items where model_id::text like '9f240001-%' or organization_template_id::text like '9f240001-%';
    delete from public.litter_planning_models where id::text like '9f240001-%';
    delete from public.litter_care_task_template_commands where template_id::text like '9f240001-%';
    delete from public.litter_care_task_templates where id::text like '9f240001-%';
...
function counts() {
  return JSON.parse(sql(`select json_build_object(
    'commands',(select count(*) from public.litter_planning_model_commands where model_id::text like '9f240001-%' or client_command_id::text like '9f240001-%'),
    'items',(select count(*) from public.litter_planning_model_items where model_id::text like '9f240001-%' or organization_template_id::text like '9f240001-%'),
    'models',(select count(*) from public.litter_planning_models where id::text like '9f240001-%'),
    'templates',(select count(*) from public.litter_care_task_templates where id::text like '9f240001-%'),
...
test("planning models enforce composition, idempotency, revision, RLS and cleanup", async () => {
  ...
  try {
    ...
    const created = await createLitterPlanningModelCore(organizationId, `${prefix}20`, { title: "E2E composed planning", species: "dog", breed: "Golden Retriever", items: [point, window] }, owner);
    expect(created).toMatchObject({ outcome: "success", revision: 1, replayed: false });
    if (created.outcome !== "success") throw new Error("model creation failed");
    expect(sql(`select count(*) from public.litter_planning_model_items where model_id=${q(created.modelId)}::uuid;`)).toBe("2");
    ...
    expect(replay).toMatchObject({ outcome: "success", modelId: created.modelId, replayed: true });
    ...
    const replaced = await replaceLitterPlanningModelCore(created.modelId, `${prefix}24`, 1, { title: "E2E changed", items: [point] }, owner);
    expect(replaced).toMatchObject({ outcome: "success", revision: 2 });
    expect(sql(`select count(*) from public.litter_planning_model_items where model_id=${q(created.modelId)}::uuid;`)).toBe("1");
    const stale = await replaceLitterPlanningModelCore(created.modelId, `${prefix}25`, 1, { title: "stale", items: [point] }, owner);
    expect(stale).toMatchObject({ outcome: "error", error: { code: "stale_revision" } });
    const concurrent = await Promise.all([setLitterPlanningModelActiveCore(created.modelId, `${prefix}26`, 2, false, owner), setLitterPlanningModelActiveCore(created.modelId, `${prefix}27`, 2, false, owner)]);
