...
async function createQualifiedApplicationFixture(supabase: SupabaseTestClient) {
  ...
  if (roleError) {
    throw new Error(`create withdrawal role: ${roleError.message}`);
  }
...
async function createDraftReservation(
  ...
  if (error) {
    throw new Error(`create withdrawal reservation: ${error.message}`);
  }
...
test("withdraws an active reservation manually without side effects", async ({
  ...
  try {
    ...
    await expect(
      page.getByText("Dossier adoptant marqué comme désisté."),
    ).toBeVisible();
