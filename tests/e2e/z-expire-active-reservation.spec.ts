...
async function createQualifiedApplicationFixture(supabase: SupabaseTestClient) {
  ...
  if (roleError) {
    throw new Error(`create expiration role: ${roleError.message}`);
  }
...
async function cleanupReservationFixture(
  ...
  if (remaining !== 0) {
    throw new Error(`cleanup expiration fixtures: ${remaining} row(s) remain`);
  }
}
...
async function createDraftReservation(
  ...
  if (error) {
    throw new Error(`create expiration reservation: ${error.message}`);
  }
...
test("expires an active reservation manually without side effects", async ({
  ...
  try {
    ...
    await expect(
      page.getByText("Dossier adoptant marqué comme expiré."),
    ).toBeVisible();
