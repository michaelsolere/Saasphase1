...
const ids = { mother: "b2000000-0000-4000-8000-000000000001", litter: "b2000000-0000-4000-8000-000000000002", template: "b2000000-0000-4000-8000-000000000003", model: "b2000000-0000-4000-8000-000000000004", ovulation: "b2000000-0000-4000-8000-000000000005", mating: "b2000000-0000-4000-8000-000000000006", window: "b2000000-0000-4000-8000-000000000007", pending: "b2000000-0000-4000-8000-000000000008", command: "b2000000-0000-4000-8000-000000000009" };
const sql = (statement: string) => runE2eSqlSync(statement);
const q = (value: string) => `'${value}'`;

function cleanup() { sql(`delete from public.litter_plan_application_commands where client_command_id=${q(ids.command)}::uuid; delete from public.litter_care_tasks where litter_id=${q(ids.litter)}::uuid; delete from public.litter_plan_items where litter_id=${q(ids.litter)}::uuid; delete from public.litter_plans where litter_id=${q(ids.litter)}::uuid; delete from public.litter_planning_model_items where model_id=${q(ids.model)}::uuid; delete from public.litter_planning_models where id=${q(ids.model)}::uuid; delete from public.litter_care_task_templates where id=${q(ids.template)}::uuid; delete from public.litters where id=${q(ids.litter)}::uuid; delete from public.animals where id=${q(ids.mother)}::uuid;`); }
...
test("instantiates corrected ovulation anchors, explicit mating and pending anchors", async () => {
  ...
  insert into public.litter_planning_models(id,organization_id,title,species,breed,revision,created_by,updated_by) values(${q(ids.model)}::uuid,${q(org)}::uuid,'B2 model','dog','Golden Retriever',1,${q(owner)}::uuid,${q(owner)}::uuid);
