...
function seed() {
  ...
  sql(`
    ...
    insert into public.reservations (id, organization_id, contact_id, application_id, litter_group_id, status, reserved_sex_preference, price_cents, currency, created_at)
    values (${q(ids.contractReservation)}, ${q(ids.organization)}, ${q(ids.contact)}, ${q(ids.application)}, ${q(ids.group)}, 'active', 'female_only', 250000, 'EUR', '2026-07-01T09:00:00Z');
    ...
    values (${q(ids.legacy)}, ${q(ids.organization)}, ${q(ids.contact)}, ${q(ids.application)}, ${q(ids.contractReservation)}, 'reservation_contract', 'uploaded', 'Contrat legacy QA', true);
